...
def api_pipeline_demo():
    ...
    return jsonify({
        ...
        "config": {
            ...
            "snow": {"source": "fixed", "thickness_m": 0.5},
            "release": {"slope_min_deg": 28, "slope_max_deg": 60, "min_area_m2": 50000},
            "simulation": {"friction_model": "samosATAuto", "mesh_cell_size_m": 25,
                           "t_end_s": 100, "res_type": "ppr|pft|pfv", "snow_density": 200,
                           "delta_th_m": 0.5, "rel_th_from_shp": True},
...
@app.route("/api/pipeline/demo2")
def api_pipeline_demo2():
    """Demo 2: full domain, all PRAs auto-delineated, 25m resolution."""
    import shutil
    import json as json_mod

    demo_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "demo")
    demo_project = os.path.join(DATA_DIR, "demo2_full_domain")
    dem_path = os.path.join(demo_project, "Inputs", "dem.tif")
    dashboard_path = os.path.join(demo_project, "Outputs", "dashboard.html")

    has_results = os.path.isfile(dashboard_path)
    has_sim = os.path.isdir(os.path.join(demo_project, "Outputs", "com4FlowPy"))

    if not has_results and not has_sim:
        if os.path.exists(demo_project):
            shutil.rmtree(demo_project)
        for d in ["Inputs", "Outputs", "Work", ".pipeline"]:
            os.makedirs(os.path.join(demo_project, d), exist_ok=True)

        # Copy DEM only (release areas will be auto-generated by step 4)
        shutil.copy2(
            os.path.join(demo_dir, "Inputs", "demo_dem.tif"),
            os.path.join(demo_project, "Inputs", "dem.tif"),
        )
    ...
    return jsonify({
        ...
        "dashboard_path": dashboard_path if has_results else None,
        "config": {
            "project": {"name": "demo2_full_domain", "dir": demo_project},
            "domain": {"from_dem": True},
            "dem": {"source": "local", "path": dem_path, "target_res_m": 25},
            "snow": {"source": "fixed", "thickness_m": 1.0},
            "release": {"slope_min_deg": 28, "slope_max_deg": 60, "min_area_m2": 5000,
                        "simplify_tolerance_m": 15, "erode_cells": 2},
            "simulation": {"model": "com4FlowPy", "flowpy_alpha": 25, "flowpy_exp": 8},
            "dashboard": {"generate": True,
                          "title": "Demo 2 - Full Domain All PRAs (FlowPy)",
                          "downsample": 1},
        },
    })
